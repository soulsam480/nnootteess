...
const tabs = computed(() => {
  const final = tabGroups.value.reduce<Record<string, [ITabGroup, NodeObject<TListNote>[]]>>(
    (groups, group) => {
      ...
      for (const edge of group.notes) {
        if (notes.value.index.has(edge)) {
          groups[group.id][1].push(notes.value.index.get(edge) as NodeObject<TListNote>);
        }
      }
      ...
      <div class="mdst-tabs-list" role="tablist" v-if="tabs.length">
        ...
        >
          ...
          <div class="tab__actions">
            <CarbonClose @click="handleRemove($event, noteId, tab.id!)" />
          </div>
        </button>
      </div>
    </template>
